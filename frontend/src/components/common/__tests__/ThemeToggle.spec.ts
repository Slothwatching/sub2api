...
describe('ThemeToggle compatibility', () => {
  it('keeps the existing theme preference and updates its accessible label after a language switch', async () => {
    ...
    expect(wrapper.get('.theme-auto').attributes('aria-pressed')).toBe('false')
    ...
    await wrapper.get('.theme-auto').trigger('click')
    expect(localStorage.getItem('theme')).toBe('system')
    expect(wrapper.get('.theme-auto').attributes('aria-pressed')).toBe('true')
    await wrapper.get('button').trigger('click')
    expect(wrapper.get('.theme-auto').attributes('aria-pressed')).toBe('false')
    wrapper.unmount()
  })
