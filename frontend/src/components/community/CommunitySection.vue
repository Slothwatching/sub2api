  ...
  <section v-if="items.length" id="community" class="space-y-5 py-6" aria-labelledby="community-title">
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <article v-for="item in items" :key="item.id" class="card flex min-w-0 flex-col items-center gap-4 p-6 text-center">
        <span class="max-w-full break-all text-xs uppercase tracking-widest text-gray-500 dark:text-dark-300">{{ item.platform }}</span>
...
const name = (item: CommunityLink) => locale.value.startsWith('en') && item.name_en.trim() ? item.name_en : item.name
</script>
